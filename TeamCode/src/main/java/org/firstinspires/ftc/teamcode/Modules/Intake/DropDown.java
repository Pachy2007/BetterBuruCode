...
public class DropDown {
    ...
    {
        if(state==State.GOING_DOWN && profile.getPosition()==state.position){state=state.nextState;}
        if(state==State.GOING_UP && profile.getPosition()==state.position){state=state.nextState;}
    }
    public void updateHardware()
    {
        profile.update();
        servo.setPosition(profile.getPosition());
        if(profile.finalPosition!=state.position)profile.setMotion(profile.getPosition() , state.position , profile.getVelocity());
    }
    public void update()
    {
        if(!ENABLE)return;
